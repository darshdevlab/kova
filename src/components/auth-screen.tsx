...
export function AuthScreen() {
  ...
  const [email, setEmail] = useState("darsh@example.com");
    ...
    <main className="auth-shell">
      <section className="auth-story" aria-labelledby="auth-title">
        ...
        <div className="auth-story-copy">
          ...
          <h1 id="auth-title">From an idea to production, without losing control.</h1>
          <p>
            Plan with stakeholders, build with agents, inspect the code, verify the result and ship from one traceable project.
          </p>
        </div>
        ...
        <div className="auth-workflow" aria-label="Kova product lifecycle">
          {["Intent", "Plan", "Build", "Verify", "Ship"].map((step, index) => (
            <div className="auth-workflow-step" key={step}>
              ...
              <strong>{step}</strong>
              {index < 4 ? <ArrowRight aria-hidden="true" /> : <Check aria-hidden="true" />}
            </div>
          ))}
        </div>

        <div className="auth-evidence">
          <div className="evidence-status"><span />Last build verified</div>
      ...
      <section className="auth-panel" aria-label="Account access">
        <div className="auth-card">
          <div className="auth-card-heading">
            <span className="auth-icon"><Sparkles aria-hidden="true" /></span>
            <div>
              <h2>{authMode === "signin" ? "Welcome back" : "Create your workspace"}</h2>
              <p>{authMode === "signin" ? "Continue building where your team left off." : "Start with a working Kova project."}</p>
            </div>
          </div>
          ...
          <form onSubmit={submit} className="auth-form">
            ...
            <label className="field">
              <span>Password</span>
              <input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete={authMode === "signin" ? "current-password" : "new-password"} minLength={8} required />
            </label>
          ...
          <div className="auth-security">
            ...
            <span>{configured ? "Supabase authentication connected" : "Demo mode active · Supabase ready"}</span>
          </div>
        </div>
      </section>
    </main>
